# LivePilot v1.21.4 — Ableton Live 12
...
- **Device Atlas** (`mcp_server/atlas/`): In-memory indexed JSON database — 5264 devices with URIs, 120 enriched with sonic intelligence (YAML), 47 with aesthetic-tagged `signature_techniques`. **7 indexes**: by_id, by_name, by_uri, by_category, by_tag, by_genre, by_pack . Reverse-index `device_techniques_index.json` (146 cross-references across 58 devices) powers `atlas_techniques_for_device`. Tools: `atlas_search`, `atlas_suggest`, `atlas_chain_suggest`, `atlas_compare`, `atlas_device_info`, `atlas_pack_info` (v1.17), `atlas_describe_chain` (v1.17 — free-text, mirror of `splice_describe_sound`), `atlas_techniques_for_device` (v1.17 — reverse-lookup), `scan_full_library`, `reload_atlas`
