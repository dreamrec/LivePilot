...
def _probe_flucoma_streams(spectral) -> bool:
    """Check whether the Max/M4L FluCoMa streams are fresh.

    FluCoMa in LivePilot is primarily a Max package feeding the M4L
    analyzer cache, not a Python dependency.  This mirrors the public
    ``check_flucoma`` tool so capability state reflects the same runtime
    truth as the analyzer tools.
    """
    if spectral is None:
        return False
    try:
        return any(spectral.get(key) is not None for key in _FLUCOMA_STREAM_KEYS)
    except Exception as exc:  # noqa: BLE001
        logger.debug("_probe_flucoma_streams failed: %s", exc)
        return False
...
def get_capability_state(ctx: Context) -> dict:
    ...
    # ── FluCoMa — Max/M4L stream first, legacy Python probe fallback ──
    flucoma_ok = _probe_flucoma_streams(spectral) or _probe_flucoma()
