...
- **Semantic Moves** (`mcp_server/semantic_moves/`, `mcp_server/sample_engine/moves.py`): 43 musical intents across 7 families (mix, arrangement, transition, sound_design, performance, device_creation, sample). `apply_semantic_move` compiles a move into concrete tool calls based on current session topology
