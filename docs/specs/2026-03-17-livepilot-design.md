...
| Tool count | 293 across 39 domains (was 104/10 at design time) | Covers full production workflows |
...
- **Stale connection detection:** MCP server detects stale connections on send failure (not pre-command ping). On socket error before send completes → reconnect once → retry. On send-completed error → fail without retry to avoid duplicate mutations. Single-client guard race on fresh reconnect is retried once with a short delay.
...
│       ├── transport.py             # 12 MCP tools
│       ├── tracks.py                # 14 MCP tools
│       ├── clips.py                 # 11 MCP tools
│       ├── notes.py                 # 8 MCP tools
│       ├── devices.py               # 12 MCP tools
│       ├── scenes.py                # 8 MCP tools
│       ├── mixing.py                # 8 MCP tools
│       ├── browser.py               # 4 MCP tools
│       └── arrangement.py           # 19 MCP tools
├── plugin/
...
│   └── CHANGELOG.md
├── tests/
│   ├── test_tools_contract.py       # Verify all 104 tools match expected signatures
...
| 11 | `get_recent_actions` | `limit?: int` | `song.undo_history` | read |
| 12 | `get_session_diagnostics` | — | Compound: connection + state health check | read |
...
| 23 | `set_group_fold` | `track_index: int, folded: bool` | `track.fold_state` | write |
| 24 | `set_track_input_monitoring` | `track_index: int, state: int` | `track.current_monitoring_state` | write |
...
| 33 | `set_clip_warp_mode` | `track_index: int, clip_index: int, mode: int, warping?: bool` | `clip.warp_mode` | write |
...
| 51 | `get_device_presets` | `track_index, device_index` | `device.presets[]` | read |
| 52 | `set_simpler_playback_mode` | `track_index, device_index, mode: int` | `simpler.playback_mode` | write |
...
| 57 | `set_scene_color` | `scene_index: int, color_index: int` | `scene.color_index` | write |
| 58 | `set_scene_tempo` | `scene_index: int, tempo: float` | `scene.tempo` | write |
...
### Arrangement & Recording (19 tools)
...
| 77 | `create_arrangement_clip` | `track_index, start_time, length, name?` | `clip_slot.create_clip()` | write |
| 78 | `add_arrangement_notes` | `track_index, clip_index, notes: [...]` | `clip.add_new_notes()` | write |
| 79 | `get_arrangement_notes` | `track_index, clip_index, from_pitch?, pitch_span?, from_time?, time_span?` | `clip.get_notes_extended()` | read |
| 80 | `remove_arrangement_notes` | `track_index, clip_index, from_pitch?, pitch_span?, from_time?, time_span?` | `clip.remove_notes_extended()` | write |
| 81 | `remove_arrangement_notes_by_id` | `track_index, clip_index, note_ids: [int]` | `clip.remove_notes_by_id()` | write |
| 82 | `modify_arrangement_notes` | `track_index, clip_index, modifications: [...]` | `clip.apply_note_modifications()` | write |
| 83 | `duplicate_arrangement_notes` | `track_index, clip_index, note_ids: [int], time_offset?` | Compound: get → add with offset | write |
| 84 | `transpose_arrangement_notes` | `track_index, clip_index, semitones: int, from_time?, time_span?` | get → modify → apply | write |
| 85 | `set_arrangement_clip_name` | `track_index, clip_index, name: str` | `clip.name` | write |
| 86 | `set_arrangement_automation` | `track_index, device_index, parameter_index, points: [...]` | `param.automation` | write |
| 87 | `back_to_arranger` | — | `song.back_to_arranger` | write |
...
```json
{
  "name": "livepilot",
  "version": "1.0.0",
  "description": "AI copilot for Ableton Live 12 — 104 MCP tools for production, sound design, and mixing",
  "author": "Pilot Studio"
}
```
...
8. **Tool Quick Reference** — 104 tools organized by domain
